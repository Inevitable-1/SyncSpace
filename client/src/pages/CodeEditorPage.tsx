    ...
    >
      ...
      >
        ...
        <div className="flex items-center gap-1">
          ...
          <ToolbarButton disabled title="Run (coming soon)">
          ...
          <button
            ...
            className={`px-2 py-1 rounded text-[10px] transition-colors ${showSidebar ? 'bg-white/10 text-white' : 'text-gray-500 hover:text-gray-300'}`}
          >
            Members
          </button>
        </div>
      ...
      <div className="flex flex-1 min-h-0">
        ...
        {showSidebar && (
          <div
            className="w-60 flex-shrink-0 border-l flex flex-col overflow-hidden"
            style={{ background: '#252526', borderColor: '#333' }}
          >
            ...
            <div className="flex-1 overflow-y-auto p-2 space-y-1">
              ...
              {connectedUsers.map((u: CodeUser) => (
                <UserItem key={u.socketId} user={u} isSelf={u.userId === user?.id} />
              ))}
            </div>
            <div className="p-3 border-t" style={{ borderColor: '#333' }}>
...
function UserItem({ user, isSelf }: { user: CodeUser; isSelf: boolean }) {
    ...
    <div className="flex items-center gap-2 px-2 py-1.5 rounded hover:bg-white/5 transition-colors">
      ...
      <div className="flex-1 min-w-0">
        <p className="text-xs text-gray-300 truncate">
          {user.userName} {isSelf && <span className="text-gray-600">(you)</span>}
        </p>
        <div className="flex items-center gap-1">
          <div
            className="w-1.5 h-1.5 rounded-full"
            style={{ background: statusColors[user.status] || '#22c55e' }}
          />
          <span className="text-[10px] text-gray-500">{statusLabels[user.status] || 'Active'}</span>
        </div>
      </div>
    </div>
  );
