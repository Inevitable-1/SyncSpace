...
export default function WorkspaceDetailPage() {
    ...
    return (
      <div className="flex items-center justify-center h-96">
        <div className="text-center">
          ...
          <p className="text-sm font-medium mb-1" style={{ color: 'var(--text-primary)' }}>Workspace not found</p>
          <p className="text-xs mb-4" style={{ color: 'var(--text-tertiary)' }}>This workspace may have been deleted.</p>
          <button onClick={() => navigate('/dashboard/workspaces')} className="btn-primary text-sm">Back to Workspaces</button>
        </div>
      </div>
    );
  }
  ...
  if (!workspace) {
    return (
      <div className="space-y-6">
        <div className="h-40 rounded-2xl animate-pulse" style={{ background: 'var(--bg-tertiary)' }} />
        <CardSkeleton />
        <CardSkeleton />
      </div>
    );
  ...
  return (
    <div className="space-y-6 pb-16">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-3 mb-2">
              <div className="w-12 h-12 rounded-xl bg-white/20 backdrop-blur-sm flex items-center justify-center border border-white/30 flex-shrink-0">
                <span className="text-2xl">{workspace.icon || workspace.name.charAt(0).toUpperCase()}</span>
              </div>
      ...
      <div className="flex items-center gap-1 p-1 rounded-xl w-fit" style={{ background: 'var(--bg-tertiary)', border: '1px solid var(--border-color)' }}>
        {tabs.map((tab) => (
          <button
            key={tab.id}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              <div className="card p-5">
                <div className="flex items-center gap-3">
                  <div className="w-11 h-11 rounded-xl flex items-center justify-center" style={{ background: 'rgba(139, 92, 246, 0.1)' }}>
                    <svg className="w-5 h-5" style={{ color: '#8b5cf6' }} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                      <rect x="2" y="3" width="20" height="14" rx="2" />
                      <path d="M8 21h8M12 17v4" />
                    </svg>
                  </div>
                  <div>
                    <p className="text-2xl font-bold" style={{ color: 'var(--text-primary)' }}>{wsRooms.length}</p>
                    <p className="text-xs" style={{ color: 'var(--text-tertiary)' }}>Rooms</p>
                  </div>
                </div>
              </div>
              <div className="card p-5">
                <div className="flex items-center gap-3">
                  <div className="w-11 h-11 rounded-xl flex items-center justify-center" style={{ background: 'rgba(59, 130, 246, 0.1)' }}>
                    <svg className="w-5 h-5" style={{ color: '#3b82f6' }} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                      <path d="M15 19.128a9.38 9.38 0 002.625.372 9.337 9.337 0 004.121-.952 4.125 4.125 0 00-7.533-2.493M15 19.128v-.003c0-1.113-.285-2.16-.786-3.07M15 19.128v.106A12.318 12.318 0 018.624 21c-2.331 0-4.512-.645-6.374-1.766l-.001-.109a6.375 6.375 0 0111.964-3.07M12 6.375a3.375 3.375 0 11-6.75 0 3.375 3.375 0 016.75 0zm8.25 2.25a2.625 2.625 0 11-5.25 0 2.625 2.625 0 015.25 0z" />
                    </svg>
                  </div>
                  <div>
                    <p className="text-2xl font-bold" style={{ color: 'var(--text-primary)' }}>{memberCount}</p>
              ...
              <div className="card p-5">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-2xl font-bold" style={{ color: 'var(--text-primary)' }}>{activities.length}</p>
                    <p className="text-xs" style={{ color: 'var(--text-tertiary)' }}>Activities</p>
                  </div>
                </div>
              </div>
            </div>
                          ...
                          <div key={activity._id || i} className="flex items-center gap-3">
                            ...
                            <div className="flex-1 min-w-0">
                              <p className="text-xs truncate" style={{ color: 'var(--text-secondary)' }}>
                                ...
                                {' '}{activity.action}
                                {activity.entityName && (
                                  <> <span style={{ color: 'var(--text-primary)' }}>{activity.entityName}</span></>
                                )}
                              </p>
              ...
              <div className="w-72 flex-shrink-0 hidden lg:block space-y-4">
                <div className="card p-5">
                  ...
                  <div className="space-y-2">
                    ...
                    >
                      <div className="w-9 h-9 rounded-lg flex items-center justify-center" style={{ background: `${gradient}20` }}>
                        <PlusIcon className="w-4 h-4" style={{ color: gradient }} />
                      </div>
                      <div>
                        <p className="text-xs font-medium" style={{ color: 'var(--text-primary)' }}>Create Room</p>
                        <p className="text-[10px]" style={{ color: 'var(--text-tertiary)' }}>Start collaborating</p>
                      </div>
                    </button>
                    <button
                      onClick={() => { setActiveTab('members'); setShowInviteInput(true); }}
                      className="w-full flex items-center gap-3 p-3 rounded-lg text-left transition-colors hover:opacity-80"
                      style={{ background: 'var(--bg-hover)' }}
                    >
                      <div className="w-9 h-9 rounded-lg flex items-center justify-center" style={{ background: 'rgba(59, 130, 246, 0.1)' }}>
                        <svg className="w-4 h-4" style={{ color: '#3b82f6' }} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                      ...
                      <div>
                        <p className="text-xs font-medium" style={{ color: 'var(--text-primary)' }}>Invite Member</p>
                        <p className="text-[10px]" style={{ color: 'var(--text-tertiary)' }}>Grow your team</p>
                      </div>
                    </button>
                  </div>
                </div>
                ...
                <div className="card p-5">
                  <h3 className="text-sm font-semibold mb-3" style={{ color: 'var(--text-primary)' }}>Invite Code</h3>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <button
                      ...
                      style={{ background: copiedCode ? 'rgba(16, 185, 129, 0.1)' : 'var(--bg-hover)', color: copiedCode ? '#10b981' : 'var(--text-secondary)' }}
        ...
        {activeTab === 'rooms' && (
          ...
          >
            <div className="flex items-center justify-between">
              ...
              <button onClick={() => setShowCreateRoom(true)} className="btn-primary flex items-center gap-2 text-sm">
            ...
            {roomLoading ? (
              ...
              <div className="card p-12 text-center">
                <div className="w-16 h-16 mx-auto mb-4 rounded-2xl flex items-center justify-center" style={{ background: 'rgba(139, 92, 246, 0.1)' }}>
                  <svg className="w-8 h-8" style={{ color: '#8b5cf6' }} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5">
                    <rect x="2" y="3" width="20" height="14" rx="2" />
                    <path d="M8 21h8M12 17v4" />
                  </svg>
                </div>
                <p className="text-sm font-medium mb-1" style={{ color: 'var(--text-primary)' }}>No rooms yet</p>
                <p className="text-xs mb-4" style={{ color: 'var(--text-tertiary)' }}>Create a room to start collaborating.</p>
                  ...
                  >
                    <div className="flex items-start justify-between mb-3">
                      <div className="flex items-center gap-3">
                        ...
                        >
                          <span className="text-white text-xs font-bold uppercase">{room.type.charAt(0)}</span>
                        </div>
                        <div>
                          ...
                          <div className="flex items-center gap-2 mt-0.5">
                            ...
                            {room.isActive && (
                              <span className="px-1.5 py-0.5 rounded text-[10px] font-medium text-white" style={{ background: '#22c55e' }}>
                                Live
                              </span>
                            )}
                          </div>
              ...
              {showInviteInput && (
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    />
                    <button onClick={handleInviteMember} className="btn-primary text-sm" disabled={!inviteEmail.trim()}>
                      Send Invite
                    </button>
                    <button
            ...
            {memberLoading ? (
                  ...
                  <div key={i} className="card p-4 animate-pulse">
                    <div className="flex items-center gap-3">
                      ...
                      <div className="flex-1 space-y-2">
                        ...
                        <div className="h-3 w-48 rounded" style={{ background: 'var(--bg-tertiary)' }} />
                      </div>
                    </div>
                  </div>
                ))}
              ...
              <div className="card divide-y" style={{ borderColor: 'var(--border-color)' }}>
                <div className="p-4 flex items-center gap-3">
                  <div className="w-10 h-10 rounded-full flex items-center justify-center text-white font-bold text-sm" style={{ background: gradient }}>
                    {user?.name?.charAt(0) || 'Y'}
                  </div>
                  <div className="flex-1">
                    <p className="text-sm font-medium" style={{ color: 'var(--text-primary)' }}>
                      {user?.name || 'You'} <span className="text-xs ml-1" style={{ color: workspace.color || '#6366f1' }}>(Owner)</span>
                    </p>
                ...
                {members.map((member: Member, i: number) => {
                  ...
                  return (
                    <div key={member._id} className="p-4 flex items-center gap-3 relative">
                      ...
                      <div className="flex-1">
                        <p className="text-sm font-medium" style={{ color: 'var(--text-primary)' }}>{memberName}</p>
                        <p className="text-xs" style={{ color: 'var(--text-tertiary)' }}>{memberEmail}</p>
                      </div>
                      <div className="flex items-center gap-2">
                        <span
                          className="px-2 py-0.5 rounded text-[10px] font-medium capitalize"
                          style={{
                            background: member.role === 'admin' ? 'rgba(139, 92, 246, 0.1)' : 'var(--bg-tertiary)',
                        ...
                        {member.status === 'suspended' && (
                          <span className="px-2 py-0.5 rounded text-[10px] font-medium" style={{ background: 'rgba(239, 68, 68, 0.1)', color: '#ef4444' }}>
                            Suspended
                          </span>
                        )}
                        <div className="relative">
                          <button
                            onClick={() => setMemberActionId(memberActionId === member._id ? null : member._id)}
                          ...
                          <AnimatePresence>
                            {memberActionId === member._id && (
                              <motion.div
                                ...
                                style={{ background: 'var(--bg-card)', borderColor: 'var(--border-color)', boxShadow: '0 4px 12px rgba(0,0,0,0.15)' }}
                              >
                                {member.role !== 'admin' && (
                                  <button
        ...
        {activeTab === 'settings' && (
          ...
          >
            <div className="card p-6 space-y-4">
              <h3 className="text-sm font-semibold" style={{ color: 'var(--text-primary)' }}>Workspace Details</h3>
              {isEditing ? (
                <>
                  <div>
                    <label className="block text-xs font-medium mb-1.5" style={{ color: 'var(--text-secondary)' }}>Name</label>
                    <input type="text" value={editName} onChange={(e) => setEditName(e.target.value)} className="input-base" />
                  </div>
                  <div>
                    <label className="block text-xs font-medium mb-1.5" style={{ color: 'var(--text-secondary)' }}>Description</label>
                    <textarea value={editDesc} onChange={(e) => setEditDesc(e.target.value)} className="input-base resize-none" rows={3} />
                  </div>
                  <div>
                  ...
                  <div className="flex gap-2 justify-end">
                    ...
                    <button onClick={handleSaveSettings} className="btn-primary text-sm" disabled={!editName.trim() || wsLoading}>
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    <div className="w-12 h-12 rounded-xl flex items-center justify-center text-2xl" style={{ background: workspace.color }}>
                      {workspace.icon || workspace.name.charAt(0).toUpperCase()}
                    </div>
                    <div>
                      <p className="text-sm font-medium" style={{ color: 'var(--text-primary)' }}>{workspace.name}</p>
                      <p className="text-xs mt-0.5" style={{ color: 'var(--text-tertiary)' }}>{workspace.description || 'No description'}</p>
                    </div>
                  </div>
                  <button onClick={() => setIsEditing(true)} className="btn-secondary text-sm">Edit</button>
                </div>
              )}
            </div>

            <div className="card p-6 space-y-4">
              <h3 className="text-sm font-semibold" style={{ color: 'var(--text-primary)' }}>Visibility</h3>
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-sm" style={{ color: 'var(--text-secondary)' }}>{editIsPublic ? 'Public' : 'Private'}</p>
                  <p className="text-xs mt-0.5" style={{ color: 'var(--text-tertiary)' }}>
                    {editIsPublic ? 'Anyone can find and join this workspace' : 'Only invited members can access this workspace'}
                  </p>
                </div>
            ...
            <div className="card p-6 space-y-4">
              ...
              <p className="text-xs" style={{ color: 'var(--text-tertiary)' }}>Share this code with others to let them join your workspace.</p>
              <div className="flex items-center gap-2">
                <div
            ...
            <div className="card p-6" style={{ border: '1px solid rgba(239, 68, 68, 0.2)' }}>
              ...
              <p className="text-xs mb-4" style={{ color: 'var(--text-tertiary)' }}>Permanently delete this workspace and all its rooms.</p>
              <button
